# Documentation Détaillée

Ce dossier contient la documentation technique détaillée du projet.

## 📚 Guides disponibles

### Pour les utilisateurs

- **[../README.md](../README.md)** - Guide principal d'installation et d'utilisation

### Documentation technique détaillée

- **[HACS_INSTALLATION.md](HACS_INSTALLATION.md)** - Guide complet installation HACS avec dépannage
- **[DOCKER_SETUP.md](DOCKER_SETUP.md)** - Configuration Docker/Swarm complète
- **[CONFIGURATION_USB.md](CONFIGURATION_USB.md)** - Configuration port USB multi-plateformes
- **[DETECTION_MODULE_LINUX.md](DETECTION_MODULE_LINUX.md)** - Détection module LCUS sur Linux
- **[MODE_IMPULSION.md](MODE_IMPULSION.md)** - Mode impulsion courte vs mode maintenu
- **[SOMFY_IZYMO.md](SOMFY_IZYMO.md)** - Utilisation avec un émetteur Somfy IZYMO io (réf. 5135163)
- **[info.md](info.md)** - Métadonnées HACS

### Pour les développeurs

- **[../CONTRIBUTING.md](../CONTRIBUTING.md)** - Guide de contribution au projet
- **[PUBLICATION_GITHUB.md](PUBLICATION_GITHUB.md)** - Guide de publication sur GitHub

---

## 🎯 Navigation rapide

**Besoin d'aide ?**
- Installation : voir [../README.md](../README.md)
- Problème Docker : voir [DOCKER_SETUP.md](DOCKER_SETUP.md)
- Port USB introuvable : voir [DETECTION_MODULE_LINUX.md](DETECTION_MODULE_LINUX.md)
- Contribuer : voir [../CONTRIBUTING.md](../CONTRIBUTING.md)
