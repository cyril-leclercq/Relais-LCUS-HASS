    ...
    async def async_step_user(
        ...
        data_schema = vol.Schema(
            {
                ...
                vol.Required(CONF_PULSE_MODE, default=DEFAULT_PULSE_MODE): vol.In({
                    PULSE_MODE_SHORT: "Impulsion courte (0.2-1s) - Pour commandes momentanées",
                    PULSE_MODE_LONG: "Maintenu (2s-2min) - Pour course complète du volet"
                }),
                vol.Required(
                    CONF_SHORT_PULSE_DURATION, default=DEFAULT_SHORT_PULSE_DURATION
                ): vol.All(vol.Coerce(float), vol.Range(min=SHORT_PULSE_MIN, max=SHORT_PULSE_MAX)),
                vol.Required(
                    CONF_LONG_PULSE_DURATION, default=DEFAULT_LONG_PULSE_DURATION
        ...
        return self.async_show_form(
            ...
            description_placeholders={
                "short_pulse_info": "⚡ Impulsion courte : Active le relais brièvement (comme un bouton poussoir). Idéal pour des volets motorisés avec leur propre électronique de contrôle.",
                "long_pulse_info": "⏱️ Maintenu : Garde le relais activé pendant toute la durée. Pour volets nécessitant une alimentation continue pendant le mouvement.",
            }
        )
...
class VoletRelaisUSBOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(
        ...
        current_short_pulse = self.config_entry.options.get(
            CONF_SHORT_PULSE_DURATION,
            self.config_entry.data.get(CONF_SHORT_PULSE_DURATION, DEFAULT_SHORT_PULSE_DURATION)
        )
        current_long_pulse = self.config_entry.options.get(
            CONF_LONG_PULSE_DURATION,
            self.config_entry.data.get(CONF_LONG_PULSE_DURATION, DEFAULT_LONG_PULSE_DURATION)
        )
